...
use nautilus_core::{collections::into_ustr_vec, string::secret::REDACTED};
...
impl InnerHttpClient {
    ...
    async fn send_request_internal<Q: serde::Serialize>(
        ...
        if redact_url {
            log::trace!("{} {REDACTED}", request.method());
        } else {
    ...
    async fn to_response_internal(
        ...
        if redact_url {
            log::trace!(
                "HTTP response from {REDACTED}: status={}",
                response.status()
            );
        } else {
            log::trace!("{response:?}");
...
mod tests {
    ...
    async fn test_http_client_redacted_url_request_removes_endpoint_from_trace_logs() {
        ...
        assert!(
            messages
                .iter()
                .any(|(level, message)| *level == Level::Trace && message == "GET <redacted>")
        );
        assert!(messages.iter().any(|(level, message)| {
            *level == Level::Trace
                && message == "HTTP response from <redacted>: status=404 Not Found"
        }));
